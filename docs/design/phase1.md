...
- `test_sample_repository.py`
  ...
  - 저장 후 재생성한 Repository 인스턴스로 다시 읽었을 때 데이터가 유지되는지 (JSON 파일 기반
    영속성 검증)
  - `find_by_id`가 존재/미존재 케이스를 올바르게 구분하는지
  - `update`가 변경 사항을 저장하는지, 존재하지 않는 `sample_id` 갱신 시 `ValueError`가
    발생하는지 (Phase 3/4에서 재고 갱신에 재사용되므로 Phase 1에서 미리 검증)
- `test_sample_controller.py`
  - 정상 등록 시나리오
  - 잘못된 수율(0, 1.5, 음수)로 등록 시 검증 실패
  - 평균 생산시간이 0 이하인 값으로 등록 시 검증 실패
  - 시료ID 또는 이름이 빈 값일 때 검증 실패
